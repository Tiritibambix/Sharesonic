...
* Playlist management: create, rename, delete playlists; add/remove tracks; play all / shuffle; add from browser (swipe right opens a frosted-glass picker with inline "New playlist" creation) or from Now Playing; **save the entire current queue as a new playlist** from the queue view's top bar
...
* Tapping the playback media-style notification brings the app to the foreground straight into Now Playing. Every player item is built with `MediaMetadata` (title / artist / album / artwork URI) — `PlayerViewModel.mediaItem()` and the service's Auto-DJ append path. Bare `MediaItem.fromUri` left the notification relying on tags ExoPlayer might parse from the stream, so it often showed "Sharesonic is running" instead of the track
* Queue management: swipe left in browser to add to queue; in the queue view, swipe right → add that track to a playlist (incl. the current track) and swipe left → remove from queue
...
* 6 built-in themes: **Velvet** (default deep purple), **Dark**, **Light**, **High-Contrast (AAA)** (pure black bg, white/yellow), **Colourblind-safe** (blue/orange, no red/green reliance), **Legacy** (neutral dark greys with amber accent). The first five mirror Velvet's CSS; Legacy is a nostalgic palette.
* Runtime accent-color override: preset swatches + HSV picker + a **Dynamic** mode that follows the current track's artwork (the same `rememberAmbientColor` vibrant seed as the Now Playing fireflies/halo) (`AccentColorSheet`), persisted in DataStore, applied via `ColorScheme.withAccent()` (`AccentOverride.kt`). Dynamic mode is resolved at the theme scope in `MainActivity` and animates `primary` app-wide on each track change, falling back to `AppTheme.defaultPrimary()` when the art yields no usable seed
...
* Floating firefly particles behind Now Playing content (`FloatingParticles.kt`): Lissajous drift, sinusoidal twinkle, tinted from the ambient seed (falls back to `MaterialTheme.colorScheme.primary` on grayscale / no-art). `baseAlpha` range is `0.42-0.80`, gradient has a 4-stop hot core (peak alpha × 1.15 clamped, then falling off) so each particle reads as a real "spark" without ever going solid enough to occlude overlapping text
* No-artwork placeholder — `ui/player/PlaceholderArt.kt` `NoArtworkThumb`: a small static waveform silhouette seeded from `song.id` (same generator as `WaveformSeekBar`), drawn in `primary @ 65 %` alpha over a `surfaceVariant @ 35 %` background. Translucent tile so the Now Playing halo + lucioles bleed through; on the mini bar and folder rows it blends into the surrounding surface. Used at the three sites where a cover would otherwise be missing (Now Playing full-screen, MiniPlayerBar, FolderBrowser file rows). No colour derived from the track
...
* Frosted-glass effects: navigation drawer, track-info dialog, playlist picker, long-press context menu and the share-expiry prompt all blur the content behind them (`Modifier.blur`, API 31+) with a dark scrim + `Surface` card (rounded 20 dp, primary-tinted border). The reusable pieces live in `ui/components/FrostedModals.kt` (`FrostedOverlay`, `FrostedCard`, `FrostedPlaylistPicker`, `FrostedShareExpiryDialog`, `FrostedTextPromptDialog`, `FrostedConfirmDialog`, `FrostedSongContextMenu`) and are shared by Folder Browser, Search results, Now Playing and every Playlist screen so their share / add-to-playlist / save-queue / delete-confirm / rename / add-songs / long-press modals behave and look identical (including inline "New playlist" creation)
...
7. **Playlists** — list of playlists with real track counts; long-press (or `⋮` on TV) opens a `DropdownMenu` with **Rename / Share / Delete**. Create / rename / delete open frosted-glass modals (`FrostedTextPromptDialog` + `FrostedConfirmDialog`) with the Scaffold blurring behind them; Share reuses the `FrostedShareExpiryDialog` and pipes into the same `ShareConfirm` route as folder / queue shares (`VelvetRepository.sharePlaylist` loads entries → delegates to `shareFilepaths`)
8. **Playlist detail** — track list; two regular `FloatingActionButton`s (Play All + Shuffle, matching FolderBrowser's FABs); swipe to remove on phone (`✕` button on TV); **drag-to-reorder on phone** — a `DragHandle` icon per row responds to `detectDragGesturesAfterLongPress`, the dragged row lifts on `zIndex(1f)` + `graphicsLayer.translationY` and follows the finger, entries are swapped in the ViewModel whenever the drag centre crosses a neighbour with an offset-compensation step so the follow stays smooth; **TV keeps a compact `↑` / `↓` arrow pair** per row (no drag on a remote). The `LazyListState` and drag state are hoisted at the screen composable so they survive Loading↔Ready transitions, and `PlaylistDetailViewModel.load()` skips the Loading transition when we already have data (both prevent the "flash to top" on reorder commit). Rename and Add songs open frosted-glass modals (`FrostedTextPromptDialog` for rename; a bespoke `FrostedCard` with search field + result list for add-songs) with the Scaffold blurring behind them. Backing methods: `moveEntry(from, to)` / `commitReorder()` / `reorderPlaylist(token, name, filepaths)` (single atomic `savePlaylist` call)
9. **Search** — pill-shaped Material You search field living in the screen body (not the `TopAppBar` title slot, which clipped it); auto-focuses on entry. A **category chip row** (All · Songs · Folders · Artists · Albums, with counts), mirroring the Velvet webapp's search tabs, filters the results; it only appears when 2+ categories matched, and the choice lives in `SearchViewModel.tab` so it survives an artist drill-down. Filename-only matches (`files`) are folded into Songs, de-duplicated, like the webapp. Results (from the native `/api/v1/db/search`) are grouped into four sections, mirroring the Velvet webapp: **Folders** (real on-disk folders whose name matched — tapping navigates straight to the server-provided `browse_path`, no path guessing), **Artists**, **Albums** and **Songs**. Tapping an **Artist** opens a track list of that artist's own songs (via `artist-folder-songs`, matched on the exact tag incl. featuring/variant spellings) — like the webapp's artist profile — rather than trying to resolve the artist to a single folder. Tapping an **Album** navigates to its folder; tapping a **Song** plays it. Song rows (both in the main search results and in `ArtistResultsScreen`) also support the same **add-to-queue / add-to-playlist** flow as the folder browser: swipe on phone (left → queue, right → context menu) or `⋮` on TV opens the shared `FrostedSongContextMenu` (Play / Add to queue / Add to playlist), and add-to-playlist reuses `FrostedPlaylistPicker`
...
Compose Navigation. Path arguments (playlist name, folder name, artist name) are **percent-encoded via `android.net.Uri.encode`** in `Screen.urlEncode`, NOT `java.net.URLEncoder`. URLEncoder is form-encoding (space → `+`, `+` → `%2B`), and Nav Compose runs `Uri.decode` on the extracted argument itself — running an additional `URLDecoder.decode` on top would turn every literal `+` in a name into a space (a real bug: a playlist named `Luana 3***+` was received as `Luana 3*** `, breaking every subsequent `/playlist/load`). Screen composables must therefore consume the argument as-is from the bundle, no second decode step.
...
* Build system: Gradle
* CI: GitHub Actions
* On every push (any branch): build a debug APK named `sharesonic-artifact-<run>.apk` (where `<run>` is the GitHub Actions run number, passed via `-PartifactRun`) and upload it as a workflow artifact. `versionCode` is also set to `<run>` so successive debug installs upgrade cleanly. Historical note: this used to be `sharesonic-artifact+<run>.apk`, but `actions/upload-artifact@v4` rejects `+` in artifact names — the upload appeared to succeed but the file was silently invalidated server-side, producing no downloadable artifact.
